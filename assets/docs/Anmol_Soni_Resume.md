# ANMOL SONI

**Cybersecurity Professional & VAPT Expert**

📍 Varanasi, Uttar Pradesh, India  
📧 anmolsoni1456@gmail.com  
📞 +91 7905826699  
🔗 Link: [LinkedIn](https://www.linkedin.com/in/anmol-soni-29b566265/) | Link: [GitHub](https://github.com/anmol1456)  
🌐 Portfolio: [anmol-soni-portfolio.onrender.com](https://anmol-soni-portfolio.onrender.com)

## PROFESSIONAL SUMMARY

Cybersecurity professional with experience in penetration testing, malware analysis, and vulnerability assessment gained through multiple internships and real-world projects. Built security tools including a malware analysis sandbox, web vulnerability scanner, and DDoS protection system. Dedicated to identifying risks, improving defenses, and helping organizations enhance their overall security posture.

## PROFESSIONAL EXPERIENCE

### **Penetration Tester Intern**  
**Deltaware Solutions** | June 2025 – July 2025  
🔗 Link: [Certificate](https://anmol-soni-portfolio.onrender.com/cert-views/cert-deltaware.html)

**Key Achievements:**
- **Conducted network-level vulnerability assessments** using Nessus, identifying critical misconfigurations and unpatched services.
- **Developed a multi-threaded Python-based password cracker** to demonstrate risk exposure in brute-force scenarios.
- **Collaborated on security hardening projects**, reducing attack surface area for client infrastructure.

### **Security Tester Intern**  
**Bornsec** | April 2025 – June 2025  
🔗 Link: [Certificate](https://anmol-soni-portfolio.onrender.com/cert-views/cert-bornsec.html)

**Key Achievements:**
- **Executed comprehensive penetration tests** on web applications, identifying 12+ critical vulnerabilities including SQLi, XSS, and IDOR.
- **Utilized Burp Suite and Nessus** for hybrid manual/automated auditing, ensuring strict compliance with OWASP Top 10 standards.
- **Authored detailed technical reports** with remediation strategies, directly improving client security posture.

### **Networking & Cybersecurity Intern**  
**Megamind IT Services** | March 2025 – August 2025  
🔗 Link: [Certificate](https://anmol-soni-portfolio.onrender.com/cert-views/cert-megamind.html)

**Key Achievements:**
- **Analyzed network traffic with Wireshark** to detect intrusion attempts and protocol anomalies.
- **Engineered Python monitoring scripts** that improved threat detection efficiency by 40%.
- **Designed social engineering simulations** that significantly increased employee security awareness and phishing resilience.

## SELF-INITIATED PROJECTS

### **Malware Analysis Sandbox**  
🔗 Link: [Repository](https://github.com/anmol1456/Malware-Analysis-Sandbox)

A secure virtualized environment utilizing custom Python scripts for behavior profiling and indicator of compromise (IOC) extraction from suspicious binaries.

**Tech Stack:** Static/Dynamic Analysis, Python, Cuckoo

### **Vulnerability Automation Suite**  
🔗 Link: [Repository](https://github.com/anmol1456/Web-Application-Vulnerability-Scanner)

High-performance Python scanner specializing in deep-crawl detection for SQLi and XSS, with integrated logic to bypass common WAF filters.

**Tech Stack:** Security Automation, OWASP 10, WAF Bypass

### **Shield Guardian (IPS/IDS)**  
🔗 Link: [Repository](https://github.com/anmol1456/DDOS-protect-system)

Real-time DDoS detection system utilizing rate-limiting and behavioral anomaly detection to proactively block volumetric attacks at the firewall level.

**Tech Stack:** IPS/IDS, Networking, Firewall Automation

## CERTIFICATIONS

**Cisco Ethical Hacker**  
September 2024  
🔗 Link: [Certificate](https://anmol-soni-portfolio.onrender.com/cert-views/cert-cisco.html)

**EC-Council SQL Injection**  
August 2024  
🔗 Link: [Certificate](https://anmol-soni-portfolio.onrender.com/cert-views/cert-eccouncil.html)

**Cyber Security Threats**  
August 2024  
🔗 Link: [Certificate](https://anmol-soni-portfolio.onrender.com/cert-views/cert-greatlearning.html)

**Ethical Hacking Bundle**  
December 2023  
🔗 Link: [Certificate](https://anmol-soni-portfolio.onrender.com/cert-views/cert-techhacker.html)

## TECHNICAL SKILLS

**Technical:**  
Vulnerability Assessment, Penetration Testing, Nmap, Burp Suite, Nessus, Wireshark, Python, Linux, SQL Injection, Social Engineering

**Soft Skills:**  
Problem Solving, Team Player, Adaptability, Communication, Time Management

## LANGUAGES
- **English** (Fluent)
- **Hindi** (Fluent)
